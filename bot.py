...
def load_allowed_admins():
    try:
        ...
        return [123456789]  # Замените на ваш user_id как начального администратора
...
async def send_welcome(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сброс флагов для предотвращения конфликтов
    context.user_data.pop('awaiting_user_id', None)
    context.user_data["awaiting_name"] = True

    # Отправка user_id всем пользователям
    ...
    keyboard = [['Управление пользователями']] if user_id in ALLOWED_ADMINS else []
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True) if keyboard else ReplyKeyboardRemove()

    welcome_message += "Привет! Я чат-бот, который может анализировать нашу переписку и искать информацию в интернете. Как тебя зовут?"
...
async def handle_message(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Клавиатура администратора по умолчанию
    admin_keyboard = [['Управление пользователями']] if user_id in ALLOWED_ADMINS else []
    default_reply_markup = ReplyKeyboardMarkup(admin_keyboard,
                                               resize_keyboard=True) if admin_keyboard else ReplyKeyboardRemove()

    # Обработка имени пользователя
    if context.user_data.get("awaiting_name", False):
        histories[chat_id]["name"] = user_input
        context.user_data["awaiting_name"] = False
        await update.message.reply_text(f"Рад знакомству, {user_input}! Задавай свои вопросы!",
    ...
    # Обработка ввода user_id для добавления пользователя или администратора
